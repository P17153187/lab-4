...
function launchConfetti() {
  ...
  const COLORS = ['#4f8ef7','#8b5cf6','#f472b6','#3ecf8e','#f59e0b','#7eb8ff','#fff'];
  ...
  requestAnimationFrame(function frame(ts) {
    ...
    for (const p of particles) {
      ...
      p.vy += 0.06;
      p.rot += p.rotV;
      const alpha = elapsed < DURATION * 0.6 ? 1
...
document.getElementById('btn-log-day').addEventListener('click', () => {
  ...
  const alreadyLogged = !!localStorage.getItem(LOG_KEY(TODAY));
  const confirmBtn = document.getElementById('btn-confirm-log-day');
  confirmBtn.textContent = alreadyLogged ? 'Update Log' : 'Mark Day as Complete';
  confirmBtn.onclick = () => {
